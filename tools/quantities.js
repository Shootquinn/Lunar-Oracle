 ...
 *   --index --write   the in-place regeneration. THIS is the boundary procedure's command.
 *             Repaired 2026-08-28 (W2-2): COUNTING_RULE.md section 4 part 3 says "re-run
 *             tools/quantities.js --index to regenerate QUANTITIES.md", and that command exits 0
 *             having changed nothing. Measured by another seat on a staged tree: 11 failures,
 *             --index, 11 failures, exit 0; --index --write, 9. A documented boundary step that
 *             reports success and leaves the correction permanently unapplied is the same
 *             container-versus-content defect this suite exists to catch, one level up, inside the
 *             counting contract itself. The DEFAULT stays dry-run -- a flag that reads as
 *             "show me the index" must not mutate the tree -- so the tool now SAYS what it did
 *             rather than exiting silently, and the contract line is routed for correction.
...
function buildIndex() {
  ...
  const L = [];
  L.push('<!-- GENERATED by `node tools/quantities.js --index --write`. Never hand-edited: COUNTING_RULE.md section 7. -->');
  L.push('');
  L.push('# QUANTITIES');
  L.push('');
...
if (OPT.index) {
  ...
  if (OPT.write) {
    ...
    // A command that changes nothing must SAY it changed nothing. Without this line, following
    // COUNTING_RULE.md section 4 part 3 verbatim looks like a successful regeneration and is not one.
    console.error('NOTE --index is the DRY RUN: ' + idx.rows +
      ' blocks printed to stdout, QUANTITIES.md UNCHANGED. The in-place regeneration is ' +
      '`node tools/quantities.js --index --write`. COUNTING_RULE.md section 4 part 3 names the ' +
      'wrong one; correction routed by W2-2, 2026-08-28.');
  }
  process.exit(0);
}
if (OPT.census) {
