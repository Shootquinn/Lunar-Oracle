...
*The count is 72, not 73.* `GATE-1` dissolved into Step 4's closing statement. It was the last row of
...
*Lunar Scenario Explorer*, `github.com/Shootquinn/lsei-lunar-scenario-explorer`, main at `f788ea2`,
...
rewrites are **not** local-only. `origin/main` reads `f788ea2`, the same ref this working copy is on,
...
measured against (`f788ea2`, `f0c976b`) rather than a bare number. And "FIXED" in the loose ends
...
Roughly 20 of the 72 sub-steps are not independent work but mandated TDD stages (test suite, outline,
...
| A8 | — | **Two more errors in `lsei/README.md`, and neither is fixed anywhere yet.** Line 12 says `literature/` is "all at one level with no subfolders"; it has eight topic folders. Line 46 says `oracle/` "holds three command-line tools" and then describes four. | The Fact-Checker at 0.5, read against the tree: eight directories under `lsei/literature/`; four top-level scripts in `lsei/oracle/` (`answer_question.js`, `render_figure.js`, `verify_answers.js`, `verify_figure.js`). | **PENDING-PUSH.** No sub-step in this plan owns another repository's README, which is why the address column is a dash; the fix is a commit to LSEI |
...
| C4 | 1.6 | **The premise of this row was wrong, and the finding is worse than it was written.** `verify_report.js` is named as an unlisted dependency of three of The Software Engineer's mechanisms. It does not live in a floating working copy — **it is in neither working copy.** `lsei/` has no `tools/` directory at all, tracked or untracked, and three files in `lsei/oracle/` reference `verify_report.js` in comments as though it were present. "Vendor it with a recorded sha, or pin that one file" is therefore not an available option: there is nothing to vendor and nothing to pin. **The dependency has to be relocated before anything can depend on it** — found elsewhere, written here, or dropped and the three mechanisms rewritten without it. That is the decision now in front of the author. | The Software Engineer, disagreement (d), for the dependency. The Fact-Checker at 0.5 for the absence: `find lsei cr-agents -name 'verify_report*'` returns nothing, `git -C lsei ls-files` lists no `tools/` path, and `lsei/` holds no `tools/` directory. | **AUTHOR**, tied to Open Question 3. 1.6 records whatever is ruled |
...
| E8 | — | **Nothing invokes the checks.** `tools/` appears nowhere in the 72 sub-steps. `check_corpus_collisions.js` and `audit_abstract_overlap.js` exist and nothing runs them. This is E1 restated with evidence: a check nothing wires up is not a mechanism. | The Systems Engineer at 0.5; 2.14's `core.hooksPath` installation covers `oracle/check_no_sources.js` only. | OPEN, **needs a sub-step**, which is why the address column is a dash |
...
| E14 | 1.4, 1.7, 2.3 | **Windows long paths break a fresh clone.** Cloning the Scenario Explorer on Windows fails: `unable to create file literature/power-and-thermal/ieee-2022-paper-sh-tcs-architecture-and-technical-challenges-update.md: Filename too long`. This project runs on Windows, and the merge makes paths longer rather than shorter — a deeper taxonomy and semantic filenames push the same limit from both ends. A bootstrap that cannot clone its own dependency is not a bootstrap. | Observed on a fresh clone during Step 0. | OPEN. Two separate fixes: 1.4 sets `core.longpaths` in the bootstrap so the clone succeeds, and a **path-length ceiling becomes a taxonomy constraint** in `literature/NAMING.md` (1.7), asserted when the taxonomy lands (2.3) |
...
| E16 | 1.x, needs a sub-step | **This project generates counts faster than it records their counting rules.** Three of the Fact-Checker's six UNSUPPORTED findings are numbers stated without the rule that produced them, and a fourth is a denominator nobody can reconcile. The Manager ruled at 0.7 that this is one common cause rather than four incidents. The echo site registry has a "what exactly is counted" column; it is not where counts are born, so a number reaches the prose before it reaches the registry. Demonstrated by the registry itself: it still read `89 / 6` after the gameplan had been corrected to 87 of 95. | The Manager at 0.7, from the Fact-Checker's UNSUPPORTED class. | OPEN. **Nothing owns the fix.** Needs a sub-step in Step 1: a count enters the registry with its counting rule at the moment it is measured, or it is not quotable. |
| E17 | 1.x, needs a sub-step | **The gameplan becomes a cold session's operating contract when Step 1 opens, and nothing schedules a re-read against that reader.** The Manager holds his F1 ruling that the TDD precondition did not fire on Step 0, and names what it cost: the Designer's "works as a briefing, fails as a worklist" is exactly the defect an audience-comprehension test would have caught before the document reached 806 lines. He declines to reopen F1 as retroactive ceremony. | The Manager at 0.7, against his own ruling. | OPEN. One row, early in Step 1 |
...
he explicitly declines to certify: 72 sub-steps is a schedule risk, and he judges it not to be an
integrity defect.
