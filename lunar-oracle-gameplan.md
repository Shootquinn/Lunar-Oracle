...
| 1.0-1.13 | 2026-08-27 | **Step 1 executed. Fourteen sub-steps, five ordered groups, one cycle.** All fourteen have deliverables in `cr_scratch/step1_*.md`, plus four addenda and three reviews. The orchestrator re-ran every load-bearing empirical claim; verdicts and the commands that produced them are in `cr_scratch/step1_orchestrator_verification.md`, which is the file a cold session should read first. Landed on disk: the corrected `.gitignore` with 18 passing probes, six checks and harnesses under `tools/`, three new directory-map rows. Frozen as specifications and not yet promoted to final paths: the answer contract (v2), the bootstrap contract, the install state record, the currency policy, `NAMING.md`, the register schema, the check register, the counting-rule contract, 32 register axes with 133 member rows, and a 211-test answering-loop suite. |
...
| # | N.M | Finding | Evidence | Status |
|---|---|---|---|---|
| E1 | 1.4, 2.14 | **A git pre-commit hook is not a mechanism**, because hooks are not cloned. Any check must be a committed script that the bootstrap wires via `core.hooksPath`. This is a dependency of A1 and A3 staying fixed. | The Systems Engineer at 0.2 and 0.5. | **CLOSED at 1.13, and the fix is not the one this row proposed.** Asserting `core.hooksPath` is set proves nothing: it can point at a directory that does not exist, or at an empty one, and git exits 0 and fires no hook in both cases. Both probed. Replaced with `git hook run pre-commit`, which goes through git's own resolver and fails when the path, the directory, the filename or the exec bit is wrong. **A second defect found the same way:** all eight files in `tools/` are committed at `100644`, so a hook committed there is inert on a Linux clone and passes on the author's machine |
