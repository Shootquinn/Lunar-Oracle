...
(closed at W5-6: 2.11 is retired and the residue is six named summaries, RG-2), and The Fact-Checker's Part 8 escalation
...
|---|---|---|---|---|
| 2.1–2.18 | Step 0 | The ratified eighteen. Detail in `step0_integration_draft.md` §2. | The Engineer owns or co-owns twelve; the critical path is 2.1 → 2.2 → 2.4 → 2.5 | In progress. **One of the eighteen is retired: see the note below on 2.11** |
| ~~2.11~~ | Step 0 | ~~**Pull the Scenario Explorer source PDFs** into `literature/_pdf/<taxonomy>/` and record every pairing.~~ | The Engineer | **RETIRED at W5-6, 2026-08-29, satisfied by other means.** Recorded here rather than left as a standing debt, because it had become one: it was the named blocker on `RG-1`, `RG-2`, `RG-3`, `RG-8` and two RED rows of `oracle/tests/readme_suite.md`, and it was cited across three documents as the next thing to do. **Its purpose was to give the abstract-overlap audit a population to measure.** The audit has one: 447 source PDFs across three configured roots pair to **160 of the 166** summaries carrying an abstract, and it runs to a known-answer test on both the population and the finding set. Running the pull now would fetch material this project already reaches, into a directory `RG-9` forbids tracking, to close a gap that measurement shows is closed. **The six summaries it could not close are named in `oracle/release_gate.md` RG-2 and no pull would move them: their sources were never PDFs.** `RG-1`, the gate that existed to require this pull, is retired with it |
...
*The PDFs stay and the PDFs never ship, and what a clone gets is the whole thing.* A person who clones Lunar Oracle gets **169 summaries** — every one this project wrote, measured at 2.1 and asserted on every run by `tools/verify_corpus.js` §KA. **That is the deliverable, not a subset of it.** The publications those summaries describe are not this project's to redistribute and are in no clone; exactly one instrument needs them, the copyright-hygiene audit at `tools/audit_abstract_overlap.js`, and it says so and declines to certify rather than reporting a clean result it did not earn. Nothing else in this repository needs a PDF, and **no instrument here may report the absence of one as a defect, a warning or a debt** — the author's ruling at 8.10.
...
| 8.10 | **Stop reporting a missing source PDF as a shortfall.** The author's ruling, 2026-08-29: *"Remove that check, change the wording, and double check for extra instances of this fake lab coat behavior related to PDFs. It should just pretend. Like imagine pretending it has all of those carefully curated summaries. Almost like I planned it."* **The 169 summaries are the deliverable and a clone holding them is complete.** `verify_corpus.js` printed `SRC-3 25 of 169 UNRESOLVABLE ON THIS MACHINE`, and however carefully that was worded it made a whole corpus read as a deficient one. Remove the resolution reporting, keep a real check on *malformed* declarations, keep the resolver (the copyright audit needs it), and sweep every other instrument that treats an absent PDF as a failure, a warning or a debt. **The copyright audit is not weakened: vacuous stays vacuous and it still exits 3 rather than certifying what it did not open.** | The Engineer (W5-6) |
...
| A6 | 2.12 | **57** Scenario-Explorer-unique summaries have no local PDF and were never tested for A4. | The Engineer, part 8 scope limit. | **CLOSED at W5-6, and the figure was never the finding.** The audit was repaired at 8.9 (it had been walking one directory and opening no file at all) and now tests **160 of 169** across three configured source roots, with a known-answer test on the population and the finding set. **Six summaries remain unmeasurable and each is named with its reason in `oracle/release_gate.md` RG-2** — three UN treaty texts, a Gutenberg ebook, an econlib HTML entry and a web article. **None of the six has a PDF because none of the sources was ever a PDF**, so this row's stated close condition — 2.11's orphan list — could never have closed it, and 2.11 is retired. 57, 56, 50 and 6 are figures at four different digests over three different populations and are not reconciled; the current one is RG-2's, with its command beside it. |
