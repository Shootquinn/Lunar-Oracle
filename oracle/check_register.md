...
C	CHK-41	oracle/reason_codes.js	generator	--render-section-5: emits oracle/answer_contract.md section 5 three-column table, which is committed between markers; and, in its default mode, it IS the single declaration of the refusal reason-code set that oracle/router/classify.js, oracle/router/acceptance.js and tools/verify_answers.js derive from	manual	n/a	The Manager Step 48 Ruling 1; answer_contract.md section 5. TWO MODES, ONE ROW, and that is section 3 own rule rather than an economy: the one-artifact-several-modes convention splits on CONSEQUENCE, and generator and fixture are both n/a, so a second row would carry no second consequence. ROW WRITTEN IN THE SAME EDIT AS THE ARTIFACT, on CHK-31 own cell -- ROW MINTED BEFORE THE ARTIFACT ... CL-1 would have failed it on the day it landed -- read in the opposite direction: oracle/**/*.js is a declared S root, so an authority landing without a row is the 25th uncovered file. Its output correctness is asserted by REF-2 and REF-3 in oracle/tests/run_suite.js and never by itself, which is what kind generator requires. Without this row the reason-code set has no declared home and the four consumers fork again	live
# END CHECKS
```
