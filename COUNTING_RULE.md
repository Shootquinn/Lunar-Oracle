...
3. Re-run `tools/quantities.js --index --write` to regenerate `QUANTITIES.md`. **`--write` is not
   optional and it is not a style choice:** `--index` alone prints the regenerated index to stdout,
   changes nothing on disk, and exits 0, so a correction that follows this step as it was written
   through version 4 leaves `QUANTITIES.md` stale while reporting success. Measured: `--index` exits
   0 having written no file; the in-place write is `--index --write`, which is what
   `tools/quantities.js` stamps into the generated header and names in its own usage block.
...
Generated by `tools/quantities.js --index --write`. Never hand-edited: a hand edit is overwritten by
the next regeneration and detected by the next check. (Same correction as §4 part 3, applied here
because it is the same dead command: `--index` without `--write` regenerates to stdout and leaves the
file untouched, and a reader who lands on this section rather than on §4 would otherwise be told the
wrong half twice.)
